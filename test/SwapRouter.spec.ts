...
import { abi as PAIR_V2_ABI } from '@skyxexchange/v2-core/artifacts/contracts/SkyXV2Pair.sol/SkyXV2Pair.json'
import { Fixture } from 'ethereum-waffle'
import { BigNumber, constants, Contract, ContractTransaction, Wallet } from 'ethers'
import { solidityPack } from 'ethers/lib/utils'
import { ethers, waffle } from 'hardhat'
import { ISkyXV2Pair, IWETH9, MockTimeSwapRouter02, MixedRouteQuoterV1, TestERC20 } from '../typechain'
...
describe('SwapRouter', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    const { weth9, poolDeployer, factory, factoryV2, router, tokens, nft } = await completeFixture(wallets, provider)
  ...
  beforeEach('load fixture', async () => {
    ;({ router, quoter, weth9, poolDeployer, factory, factoryV2, tokens, nft } = await loadFixture(swapRouterFixture))
  ...
  async function createV2Pool(tokenA: TestERC20, tokenB: TestERC20): Promise<ISkyXV2Pair> {
    await factoryV2.createPair(tokenA.address, tokenB.address)

    const pairAddress = await factoryV2.getPair(tokenA.address, tokenB.address)
    const pair = new ethers.Contract(pairAddress, PAIR_V2_ABI, wallet) as ISkyXV2Pair
  ...
  describe('swaps - v2', () => {
    let pairs: ISkyXV2Pair[]
    let wethPairs: ISkyXV2Pair[]
