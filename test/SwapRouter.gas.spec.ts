...
import { ISkyXV3Pool, IWETH9, MockTimeSwapRouter02, TestERC20 } from '../typechain'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [ISkyXV3Pool, ISkyXV3Pool, ISkyXV3Pool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, ISkyXV3PoolABI, wallet)) as [
      ISkyXV3Pool,
      ISkyXV3Pool,
      ISkyXV3Pool
    ]
  ...
  let pools: [ISkyXV3Pool, ISkyXV3Pool, ISkyXV3Pool]
  ...
  async function exactInput(
    ...
    return router.connect(trader)['multicall(uint256,bytes[])'](1788290550, data, { value })
  }
  ...
  describe('#exactInput', () => {
    ...
    it('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestSkyXV3Callee')
    ...
    it('2 trades (via router)', async () => {
      ...
      await snapshotGasCost(router.connect(trader)['multicall(uint256,bytes[])'](1788290550, data))
    })
