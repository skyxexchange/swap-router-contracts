...
import '@skyxexchange/v2-core/contracts/interfaces/ISkyXV2Callee.sol';
import '@skyxexchange/v2-core/contracts/interfaces/ISkyXV2Pair.sol';
import '../libraries/SkyXV2Library.sol';
...
contract TokenValidator is ITokenValidator, ISkyXV2Callee, ImmutableState {
    string internal constant FOT_REVERT_STRING = 'FOT';
    // https://github.com/skyxexchange/v2-core/blob/1136544ac842ff48ae0b1b939701436598d74075/contracts/SkyXV2Pair.sol#L46
    ...
    function _validate(
        ...
        address pairAddress = SkyXV2Library.pairFor(this.factoryV2(), token, baseToken);
        ...
        (, bytes memory returnData) = address(pairAddress).call(abi.encodeWithSelector(ISkyXV2Pair.token0.selector));
        ...
        ISkyXV2Pair pair = ISkyXV2Pair(pairAddress);
    ...
    function skyxV2Call(
        ...
        ISkyXV2Pair pair = ISkyXV2Pair(msg.sender);
