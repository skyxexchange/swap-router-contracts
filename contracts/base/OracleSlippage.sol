...
import '@skyxexchange/v3-periphery/contracts/base/PeripheryImmutableState.sol';
import '@skyxexchange/v3-periphery/contracts/base/BlockTimestamp.sol';
import '@skyxexchange/v3-periphery/contracts/libraries/Path.sol';
import '@skyxexchange/v3-periphery/contracts/libraries/PoolAddress.sol';
import '@skyxexchange/v3-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getBlockStartingAndCurrentTick(ISkyXV3Pool pool)
        internal
        view
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view virtual returns (ISkyXV3Pool pool) {
        pool = ISkyXV3Pool(PoolAddress.computeAddress(poolDeployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
        ...
        returns (int256 syntheticAverageTick, int256 syntheticCurrentTick)
    {
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            ISkyXV3Pool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = ISkyXV3Pool(pool).slot0();
            }
