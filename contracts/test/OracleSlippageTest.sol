...
contract OracleSlippageTest is OracleSlippage {
    mapping(address => mapping(address => mapping(uint24 => ISkyXV3Pool))) private pools;
    ...
    function getPoolAddress(
        ...
        address tokenB,
        uint24 fee
    ) internal view override returns (ISkyXV3Pool pool) {
        pool = pools[tokenA][tokenB][fee];
    }

    function testGetBlockStartingAndCurrentTick(ISkyXV3Pool pool)
        external
        view
